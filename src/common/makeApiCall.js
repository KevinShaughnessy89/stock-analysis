...
export async function makeApiCall(config, params = {}, data = {}) {
  try {
    ...
    if (['GET'].includes(config.method) && Object.keys(params).length > 0) {
      queryParams = Object.fromEntries(
        Object.entries(params)
          .filter(([key, value]) => config.params[key] && config.params[key] === true)
      );
    }
    ...
    let dataFields;
    if (['POST', 'PUT'].includes(config.method) && Object.keys(data).length > 0) {
      dataFields = Object.fromEntries(
        Object.entries(data)
          .filter(([key, value]) => config.data[key] && config.data[key] === true)
      );
    }
    ...
    if (config.responseHandlers && config.responseHandlers[response.status]) {
...
export async function makeDetailedApiCall(config, params = {}, data = {}) {
  ...
  try {
    ...
    if (config.responseHandlers && config.responseHandlers[response.status]) {
      const handledResponse = config.responseHandlers[response.status](response);
