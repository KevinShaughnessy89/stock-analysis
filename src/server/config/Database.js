...
class Database {
	...
	async connect() {
		if (!this.client) {
			...
			try {
				...
				console.log(
					`Connected to MongoDB database: ${this.databaseName}`
				);
			} catch (error) {
	...
	async setupDatabase(params) {
		try {
			...
			console.error(
				`Error creating database ${this.databaseName}: `,
				error
			);
		}
	}
	...
	async createTimeSeriesCollection(collectionName, params) {
		try {
			const result = await this.db.createCollection(
				collectionName,
				params
			);
			return result;
	...
	async deleteDocuments(collectionName, filter, options = {}) {
		try {
			...
			console.error(
				"Error deleting document(s) in collection: ",
				collectionName
			);
		}
	}
	...
	async query(queryConfig, params = {}) {
		...
		const updateNestedObject = (obj, params) => {
			for (const [key, value] of Object.entries(obj)) {
				if (
					...
					updateNestedObject(value, params);
				}
		...
		processedPipeline.forEach((stage) => {
			updateNestedObject(stage, params);
		});
